...
export default function ResourceTooltip({ resources = {}, topicName, onEdit }) {
  ...
  const hasAnyLink = RESOURCES.some((r) => resources[r.key])
    ...
    <div className="resource-tooltip-wrap" ref={wrapRef}>
      ...
      >
        {RESOURCES.map(({ key, icon, label }) => (
          <span
            key={key}
            className={`resource-pip ${resources[key] ? 'resource-pip--set' : 'resource-pip--empty'}`}
            title={`${label}: ${resources[key] ? 'set' : 'not set'}`}
          >
            {icon}
          </span>
        ))}
